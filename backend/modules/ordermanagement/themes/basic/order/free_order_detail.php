	...
	<table id="ticket_center_table">
				...
				<?php if($vv != null&& $vv['price_type']!=1){?>
					...
					<?php if($num==0){?>
						...
						<td><div class="btn"><input  type="button" onclick="seainfo('<?php echo (isset($_GET['order_serial_number'])?$_GET['order_serial_number']:'').','.$v['cabin_price'].','.$v['tax_price'].','.$vv['passport_number'].','.$v['voyage_code'].','.$v['cabin_type_code'].','.$v['cabin_name']?>');" value="edit"></div></td>
					</tr>
					<?php }?>
					<?php endforeach;?>
					<?php }?>
...
<script>
   function seainfo(info){
	   var info=info.split(",");
	   location.href="<?php echo Url::toRoute(['cabin_detail']);?>&order_serial_number="+info[0]+"&cabin_price="+info[1]+"&tax_price="+info[2]+"&passport_number="+info[3]+"&voyage_code="+info[4]+"&cabin_type_code="+info[5]+"&cabin_name="+info[6];
	   }

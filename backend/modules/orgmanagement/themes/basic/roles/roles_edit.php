...
<div class="r content" id="user_content">
    ...
    <a href="#"><?php echo yii::t('app','Roles_edit')?></a></div>
    ...
    <div class="searchResult">
		...
		<?php
			$form = ActiveForm::begin([
					'action' => ['rolesedit','name'=>$roles_name],
					'method'=>'post',
					'id'=>'roles_val',
					'options' => ['class' => 'roles_edit'],
		...
		<input type="hidden" id="item_list" name="item_list" value="" /> 
			<p>
				<label id="roles_input">
					<span class='max_l'><?php echo yii::t('app','Name')?>:</span>
					<input type="text" id='name' name='name' value="<?php echo $roles_name?>"></input>
			...
			<div class="searchResult selectBox" style="margin-top:20px;margin-left:120px;margin-bottom:20px;">
				<div class="l selectList">
					...
					<ul  id="cabin_left_ul">
					<?php foreach ($roles_result  as $k=>$v){?>
					<?php if($v['parent']!=$roles_name){?>
				...
				<div class="l selectList">
					...
					<ul id="cabin_right_ul">
						<?php foreach ($roles_result as $k=>$v){?>
						<?php if($v['parent']==$roles_name){?>
						<li><span><input type="checkbox" name="cabin_right_ids[]" value="<?php echo $v['name']?>"></span><span class="text"><?php echo $v['description']?></span></li>
						<?php }}?>
...
window.onload = function(){ 
	...
	$("form#roles_val").submit(function(){
		var old_name = "<?php echo $roles_name;?>";

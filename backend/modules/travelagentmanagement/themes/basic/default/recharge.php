...
<div class="r content">
			...
			<form method="post">
				<div>
					<div>
						<label>
						<input type="hidden" name="travel_agent_id" id="travel_agent_id">
							<span><?php echo yii::t('vcos', 'Agent Name:')?></span>
							<input type="text" style="border:#B0B0B0 1px solid" id="travel_agent_name2" name="travel_agent_name" readonly="readonly" class="doubleWidth"></input>
						</label>
						<label>
							<span><?php echo yii::t('vcos', 'Balance:')?></span>
							<input type="text" id="current_amount" name="current_amount" style="border:#B0B0B0 1px solid" readonly="readonly"></input>
						</label>
					</div>
					<div>
						<label>
					...
					<div>
						...
						<table>
							...
							<?php foreach ($pagedata as $k=>$v):?>
								<tr>
									<td><?php echo $k+1?></td>
									<td><span style="cursor:pointer;" title="<?php echo yii::t('vcos', $v['remarks'])?>"><?php echo yii::t('vcos', $v['travel_agent_code'])?></span></td>
									<td><span style="cursor:pointer;" title="<?php echo yii::t('vcos', $v['remarks'])?>"><?php echo yii::t('vcos', $v['travel_agent_name'])?></span></td>
									<td><?php echo yii::t('vcos', $v['recharge_amount'])?></td>
								    <td><?php echo yii::t('vcos', $v['username'])?></td>
									<td><?php echo yii::t('vcos', $v['recharge_time'])?></td>
									<?php endforeach;?>
								</tr>
							</tbody>
						</table>
...
$(function(){
	...
	if(travel_agent_id==''){
	alert("Please select to top-up agents ");
	return false;
	}
	var reg=/^\d+(\.\d{2})?$/;
	var recharge=$("#recharge").val();
	if(!reg.test(recharge)){
     alert("Can only enter two decimal places");
     return false;
		}

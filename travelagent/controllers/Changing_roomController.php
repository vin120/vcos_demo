...
class Changing_roomController  extends Controller
{
	...
	public function actionChangeroom()
	{
		...
		$count=sizeof($data);
/* 		$voyagesql="select id,voyage_code from v_c_voyage";
		$voyage=\Yii::$app->db->createCommand($voyagesql)->queryAll();//航线
		...
		for($i=0;$i<sizeof($data);$i++){
			...
			$order_cabin=implode('\',\'',$order_cabin);
			$changcabinsql="select distinct vc.cabin_name, vc.cabin_code,vc.cabin_name from v_c_voyage_cabin vc join v_c_voyage v on v.id=vc.voyage_id join v_voyage_order_detail vod on vod.voyage_code=v.voyage_code and vod.cabin_type_code=vc.cabin_code
			 where vc.voyage_id='$voyage_id' and cabin_code='$voyage_cabin' and vc.cabin_name not in('$order_cabin')";
	...
	public function actionGet_changeroom_page(){
  		$voyage_code=isset($_POST['voyage_code'])?$_POST['voyage_code']:'';
		$cabin_name=isset($_POST['cabin_name'])?$_POST['cabin_name']:'';
